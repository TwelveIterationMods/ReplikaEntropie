...
public class CobblescrapScreen extends AbstractContainerScreen<CobblescrapMenu> {
    ...
    private final FluidTankDisplay waterTankRenderer = new FluidTankDisplay(
            SimpleProgressRenderer.vertical(BACKGROUND, 256, 256).pos(17, 18).size(16, 48).uv(176, 28),
            17, 18, 16, 48,
            Component.translatable("block.minecraft.water"));
    private final FluidTankDisplay lavaTankRenderer = new FluidTankDisplay(
            SimpleProgressRenderer.vertical(BACKGROUND, 256, 256).pos(143, 18).size(16, 48).uv(192, 28),
            143, 18, 16, 48,
    ...
    protected void extractTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        if (waterTankRenderer.extractFullTooltip(graphics, font, leftPos, topPos, mouseX, mouseY)) {
            return;
        }

        if (lavaTankRenderer.extractFullTooltip(graphics, font, leftPos, topPos, mouseX, mouseY)) {
            return;
        }
